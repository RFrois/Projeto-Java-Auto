...
public class ProgramaAutoCenter {
...
//			cria��o de lista  direto no metodo main com a classe Pacas 		
...
//			for (int i=1; i<5; i++) {
		...
		while (!escolha.equalsIgnoreCase("8")) {
			...
			if ("1".equalsIgnoreCase(escolha)) { // cadastro de pe�as
...
//					System.out.println("Nova Pe�a: S/N ");
...
//				}
			...
			} else if ("3".equalsIgnoreCase(escolha)) { // Endere�o
			...
			} else if ("4".equalsIgnoreCase(escolha)) { // Contato
...
//					contato2.setCelular(entrada.nextLine());  //manter comentado
			...
			} else if ("6".equalsIgnoreCase(escolha)) { // servi�o
				...
				while ("S".equalsIgnoreCase(inserirPeca)) { // sempre que comparar um valor fixo em uma vare�vel colocar
															// o valor
															// fixo a esquerda
					Pecas peca = new Pecas(); // Inst�nciar o objeto, utilizando a classe Pecas para receber as
												// informa��es de peca

					if (listaPecasPrint == null || listaPecasPrint.isEmpty()) {
						System.out.println("N�o tem pe�a cadastrada. ");
						inserirPeca = "N";
					} else {
						for (Pecas ss : listaPecasPrint) { // for para printar nome, valor e codigo da pe�a

							System.out.println(ss.getIdPeca() + " " + ss.getPeca() + " " + ss.getValorUnitario());
						}
						System.out.println("Digite ID: ");
						novaPeca = (entrada.nextInt());
						entrada.nextLine();

						for (Pecas s3 : listaPecasPrint) {
							if (s3.getIdPeca() == novaPeca) {
								peca.setPeca(s3.getPeca());
								peca.setValorUnitario(s3.getValorUnitario());
							}
						}
						...
						servico.setValorTotal(servico.getValorTotal() + peca.getValor()); // calculo do atributo do
																							// objeto peca e
																							// atributo do objeto
																							// servico
...
//						servico.getPecas().add(peca); // populando a lista Pecas na classe servico(OrdemServico)
						...
						System.out.println("Deseja mais Pe�as?:(S/N) ");
						inserirPeca = entrada.nextLine();

						System.out.println("servi�o executado: ");
						servico.setServico(entrada.nextLine());

						System.out.println("Valor de m�o de obra: ");
						servico.setValorServico(entrada.nextDouble());
						entrada.nextLine();

						servico.setValorTotal(servico.getValorTotal() + servico.getValorServico()); // calculo de pe�as,
																									// e
																									// valor de servi�o
						System.out.println("Servi�o executado: " + servico.getServico() + "\nValor toral: " + servico.getValorTotal());
//					System.out.println("Valor Total: ");
//					servico.setValorTotal(entrada.nextDouble());
					}
				}
			}else if ("7".equalsIgnoreCase(escolha)) {
...
//					System.out.println("E-mail: " + contato1.getEmail());
				...
				for (Endereco e1 : cliente.getEndereco()) { // for (Classe do objeto, var do for : classe e objeto de da
															// instancia onde est� indo o get
					System.out.println("Tipo de endere�o: ".concat(e1.getTipoEndereco() == null ? " " : e1.getTipoEndereco()));
					System.out.println("Logradouro: ".concat(e1.getLogradouro() == null ? " " : e1.getLogradouro()));
					System.out.println("N�mero: ".concat(e1.getNumero() == null ? " " : e1.getNumero()));
					System.out.println("Complemento: ".concat(e1.getComplemento() == null ? " " : e1.getComplemento()));
					System.out.println("CEP: ".concat(e1.getCep() == null ? " " : e1.getCep())); // Operador tern�rio (condi��o
																									// ? else:)
				}

				for (CarroModelo car1 : cliente.getCarro()) {
					System.out
							.println("Tipo de ve�culo: ".concat(nomeMaiusculo(car1.getTipo() == null ? " " : car1.getTipo())));
				...
				System.out.println("Tipo de Servi�o: ".concat(servico.getTipo()));
				for (Pecas os1 : servico.getPecas()) {
					System.out.println("Pe�a Utilizada: ".concat(os1.getPeca()));
					System.out.println("Qantidade: " + (os1.getQuantidade()));
					System.out.println("valor Unit�rio: " + (os1.getValorUnitario()));
					System.out.println("Valor Pe�as: " + (os1.getValor()));
				}
				System.out.println("servi�o executado: ".concat(servico.getServico()));
				System.out.println("Valor de m�o de obra: " + (servico.getValorServico()));
